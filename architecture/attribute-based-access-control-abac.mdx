...
Ubicloud uses <Link title="Rodauth" url="https://github.com/jeremyevans/rodauth"/> for authentication. For authorization, we need to implement a system that gives users the flexibility they need to access various resources across different cloud services on the cloud.
...
<Link title="Tailscale has a great blog post" url="https://tailscale.com/blog/rbac-like-it-was-meant-to-be/"/> that describes security policies, RBAC, and ABAC in more detail.

All three hyperscalers seem to be moving towards the ABAC model. You can read more about <Link title="AWS" url="https://docs.aws.amazon.com/IAM/latest/UserGuide/introduction_attribute-based-access-control.html"/>, <Link title="Azure" url="https://learn.microsoft.com/en-us/azure/role-based-access-control/conditions-overview"/>, and <Link title="GCP" url="https://cloud.google.com/iam/docs/conditions-overview"/>'s ABAC designs on their websites. These designs rely on conditional expressions and are more complex than a clean-sheet ABAC design. This is probably because all hyperscalers need to be backwards compatible with their RBAC designs. We'd like to provide something much simpler, both in design and implementation.
...
- It's simple. We get an ABAC implementation in 10 lines of SQL. In fact, <Link title="the entire file" url="https://github.com/ubicloud/ubicloud/blob/main/lib/authorization.rb"/> that implements our authorization policy is 130 lines of code.
...
We also expect our ABAC design to evolve over time. If you have any questions or feedback about this design document, we'd love to hear from you. Please start a conversation on GitHub discussions or reach out to us at <Link title="info@ubicloud.com" url="mailto:info@ubicloud.com"/>.
