...
Managing your Kubernetes clusters from the command line is now more powerful. We've extended our `ubi` CLI with new commands to `list`, `show`, `destroy`, and retrieve the `kubeconfig` for your clusters. These additions provide full CLI-based lifecycle management for your Kubernetes resources, streamlining automation and daily operations.
