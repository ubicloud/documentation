...
<Note>
    This content is heavily inspired by DHH’s demo at <Link url="https://kamal-deploy.org"/>. We will use similar steps when deploying to Ubicloud.
</Note>
...
Now, we need to set up a dockerhub repository to store and fetch our images. For that, you need to create an account at <Link url="https://hub.docker.com"/>.
Once you have the account, please create a new repository named “blog” in the docker hub page and generate a new key at <Link url=" https://app.docker.com/settings/personal-access-tokens"/> and paste it to the .env file as shown below.
