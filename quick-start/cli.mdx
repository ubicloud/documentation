...
You can download the latest release of `ubi` from [Ubicloud's GitHub releases page](https://github.com/ubicloud/ubicloud/releases). We offer downloads of `ubi` for the following operating systems and platforms:

* Linux: amd64/x86_64/x64, 386/x86, arm64/aarch64
* MacOS/Darwin: arm64(Apple silicon), amd64(Intel)
* Windows: amd64/x86_64/x64, 386/x86, arm64

All downloads are single-file executables with no dependencies.  The do not need to be installed.

As `ubi` is part of Ubicloud, it is open source, and [available for review](https://github.com/ubicloud/ubicloud/blob/main/cli/ubi.go).  It is a small program is written in [Go](https://go.dev), and if we do not already provide builds for your platform, you can build `ubi` yourself.  You can also contact us at [support@ubicloud.com](mailto:support@ubicloud.com) and ask us to provide builds for your operating system and platform.
...
* `vm ssh`: connects to virtual machine via `ssh`
* `vm sftp`: connects to virtual machine via `sftp`
* `vm scp`: copy file/directory from local computer to VM or from VM to local computer via `scp`
* `pg psql`: connect to PostgreSQL database via `psql`
* `pg pg_dump`: dump single PostgreSQL database using `pg_dump`
* `pg pg_dumpall`: dump entire PostgreSQL database cluster using `pg_dumpall`
