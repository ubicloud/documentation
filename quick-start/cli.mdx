...
The other type of subcommand used in `ubi` is when the subcommand follows the object reference (either location/object-name or object-id).  Here's an example of that style of subcommand to create a PostgreSQL database:

```
$ ubi pg eu-central-h1/my-pg-database-name create
PostgreSQL database created with id: pg1s9nhxjjej5na985hzp9wws4
```

As no options were given, this will create a PostgreSQL database with the default settings.
