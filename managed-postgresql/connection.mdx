...
Ubicloud PostgreSQL instances are secured with TLS using self-signed
certificates. By default, **TLS v1.3** is the minimum supported version. We
recommend using at least `SSLMODE=require` when connecting applications to your
instance, to ensure that the connection is encrypted.

For production workloads, it is recommened that you are connect to your instance
